...
def autodetect_port():
    try:
        ...
        if sys.platform.startswith('win'):
            if not ports:
                sys.exit("Nie znaleziono portów COM. Podłącz radio przez USB i upewnij się, że tryb USB to VCP/Serial.")
            for p in ports:
                ...
                if any(kw in desc for kw in ["STM", "Virtual", "EdgeTX", "RadioMaster", "CDC"]):
                    ...
                    return p.device
            print(f"Wykryte porty: {[(p.device, p.description) for p in ports]}, używam {ports[0].device}")
            return ports[0].device
    except Exception:
        pass
    candidates = sorted(glob.glob("/dev/ttyACM*") + glob.glob("/dev/ttyUSB*"))
    if not candidates:
        sys.exit("no serial port found; pass --port explicitly (np. --port COM3)")
    if len(candidates) > 1:
